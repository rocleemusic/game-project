...
> **Note (post-restructure, 2026-07-26).** "Build GDD section N" citations below use pre-v5 numbering and do not match the current structure. See [`../../gdd/CONTEXT.md`](../../gdd/CONTEXT.md) for where this content lives now — the dev-crew's content agent is [`../../gdd/11-ai-agents-and-pipeline.md`](../../gdd/11-ai-agents-and-pipeline.md).
...
### 2.1 One thought per turn; deflate the follow-through
Keep lines short — a median of **5–7 words**, most under 15, and a turn may run two clauses
when the second is the payload. When a beat is heavy, exit light. Never let an emotional
landing double down on itself; the follow-through takes air *out*, it does not add weight
(`e01`, `e14`).

> **Amended 2026-08-06.** This read "One clause per line… usually under ten words." Two
> corrections, both from a measured corpus of 4,735 human-transcribed turns
> ([`../dialogue-corpus/README.md`](../dialogue-corpus/README.md)): the unit is a **thought,
> not a clause** — "one clause" came from an un-diarized caption transcript and is a subtitle
> box, not a cadence — and **long turns are real.** About **6 turns per part run over 40
> words** in every work measured, and roughly one per part exceeds 75. They carry
> **information**: exposition an explainer with standing is delivering, instruction, or a
> confession answering a question. **Never grief** — grief is short fragments divided by
> silence, and in this game that silence is an `action` slot. "Under ten words, always" is the
> belief that produced dialogue in which nobody ever held the floor. See
> `../../narrative-pipeline/register.md` for the sanctioned-run rule and its ceiling.
...
- **[+refresh] Carry story through space and affordance, not signage.** One evocative image
  ...
  dialogue. This is where the *essence echoes* should live first: in the environment, before
...
- **Plant low, beside plot-inert material.** Seed the essence-echo detail in a moment of
...
flat register is a **shared grammar, not a shared voice.** Every **carded** NPC obeys §2 (short,
...
> **Walk-ons are exempt from §2 (added 2026-08-06).** "Every NPC" above now reads **every
> carded NPC**, because a walk-on has no card — no essence, no arc, no bond — and so nothing
> sets its length or its warmth. Left on the world dialect, an uncarded villager comes out
> sounding like the guarded protagonist, which is a defect caught on a line reading *"Loaf on
> my step Tuesday morning. I never ordered it."* where the character had nothing to hide.
>
> **Walk-ons run longer, warmer and unguarded, and deflect-not-name does not apply to them.**
> They may say plainly that they were pleased or grateful. The deflection rule exists because a
> deep soul's unsaid feeling is the thing the player works out — a walk-on has nothing to work
> out, so withholding reads as evasiveness they have no reason for. **This does characterisation
> for free:** a soul's terseness only reads *as* terseness when the people around it are not
> terse. A village where everyone is clipped has no protagonist, only a house style. Band in
> `../../narrative-pipeline/register.md`; the class is `pipeline.md` step 2.
...
*visual* expression the scene-comp agent and art director can execute [`coverage-map.md` §3.11]. This is
...
- **Interactive sequencing is better-covered than before, still not closed.** Every voice source is
  *linear* media, but the refresh added order-independence craft — encounter-over-quest scenes that
  ...
  causality), and wire each essence-echo so it lands whichever year/room the player hits first.
