...
sources:
  ...
  - knowledge-base/coverage-map.md
  - GATE-2-review.md
...
> **⚠ Refined by Phase 3 (2026-07-19)** — the current source of truth on the following now lives in `gdd/` ([`11-ai-agents-and-pipeline.md`](../../gdd/11-ai-agents-and-pipeline.md), [`12-technical-overview.md`](../../gdd/12-technical-overview.md), [`10-audio.md`](../../gdd/10-audio.md)); where they conflict, `gdd/` wins:
> - **Terminology:** "emergent-partner" / "solidification" → **bond level** (partner = the *deepest bond*, an emergent possibility, not a prescribed goal) — see the H1 canonical terminology note.
> - **Live mode (H10):** not a single in-run call — the pipeline **re-runs between runs (boot / run-end), player-BYOK, never during a run**; the deepest-bond "wow" beat is one of its outputs.
> - **Audio-Tag Agent (H15):** the `<Entity>_<AnimVerb>_<State>` string + mirrored-tree is superseded by **Unreal GameplayTags + a tag→asset library + direct Wwise events** (§7 I/O below updated to match).

> **Note (post-restructure, 2026-07-26).** "Build GDD section N" citations below use the pre-v5 three-doc structure-model numbering (game-30/32 era) and do not match the current structure. See [`../../gdd/CONTEXT.md`](../../gdd/CONTEXT.md) for where this content lives now.

# Dev-Crew Architecture — the Five-Worker Pipeline + Orchestrator

> **⚠ SUPERSEDED IN FULL (Roc, 2026-08-09).** This document is history, not contract. The live architecture is `../../narrative-pipeline/agents/README.md` (seats), `../../narrative-pipeline/pipeline.md` (procedure) and `../../gdd/11-ai-agents-and-pipeline.md` (roster). Read this only for how the shape was arrived at. Specific reversals below.
>
> **Earlier banner (rulings 2026-08-08 / 2026-08-09).** Two I/O contracts below declare `suit_tag` as an enum — the field is retired (`gdd/07-cast.md`). This doc also assigns the NPC codex to the Narrative Architect; the codex is seeded by Intake, ratified at Roc's gate, transcribed by the Orchestrator and verified by `tools/codex-lint.mjs` (`gdd/11-ai-agents-and-pipeline.md`). The analysis below stands as history.


**How to use this.** This is the content behind [`../../gdd/11-ai-agents-and-pipeline.md`](../../gdd/11-ai-agents-and-pipeline.md) — the dev-crew pipeline roster the GDD hands off to agents. It names five worker agents plus
...
[`coverage-map.md` §3.4]; where a claim rests on a single thin source it is flagged inline and re-listed in
...
agents (H10 + the deepest-bond "wow"), which are a separate architecture for **Build §5a** — a hard
...
> **Revised after the live proof run (2026-07-25).** The crew was **run**, not described: a 5-arm model
> benchmark then a full stage-2 generation of one soul (the Giver), 27 agents across two phases. Evidence
> and full accounting in [`../../pipeline-runs/2026-07-25-giver/RESULTS.md`](../../pipeline-runs/2026-07-25-giver/RESULTS.md).
> **The architecture held on every claim it makes** — 17 calls, zero worker-to-worker edges, all flags
> routed up through the orchestrator, three real human gates, nothing self-approved. What strained was
> **revision governance, not the architecture**: 79% of the run's tokens went to revisions, 3.8× the
> generation they corrected. Ten specification changes follow from the run; the ones that change *this*
> document are:
>
> 1. **The invariant set is eight checks, not six** *(count as of this note; the list has since grown — the live contract is `narrative-pipeline/guardrails.md`, which states no count for exactly this reason)* — slot typing is added (§6), and the voice-register
>    check splits into three line items, the third being **warmth invariance**: where a card sets a
>    register *spread*, the spread governs tempo and uptake only, and a "correctly flat" line that reads
>    brusque or transactional is a defect. Neither the Architect nor the Verifier caught this; the human
>    gate did.
> 2. **The Verifier is criteria-bound, and that is now stated as a property rather than assumed away.**
>    It caught every defect it had a check for — including reversing two of its own prior clearances once
>    new criteria arrived — and nothing it did not. A check absent from the set is a class of defect
>    nobody catches, so it gains an **`uncovered`** flag type for naming a defect no check covers.
> 3. **The revision cap binds every worker, every flag type, and does not reset at a human gate**
>    (`../../narrative-pipeline/pipeline.md` step 13). The prior cap bound only prose flags to the Content
>    Agent, which is how the 79% happened. **The cap is a budget control as much as a process one.**
> 4. **Content items carry `slot_type`** (`dialogue` | `action` | `object`), which sets the word ceiling
>    and decides what a slot may legally contain (§5, §6 I/O sketches). Its absence let a seed be specced
>    into a slot with no legal rendering, burning four Content passes.
> 5. **The persona_card gains `age_band`, role-side** — re-dealt each life like the job (§4).
> 6. **Player choice is not expressible and is a named gap.** §6 of `pipeline.md` assumes the scene graph
>    carries branch logic, but no unit lets the Content Agent author the player-facing options. Unspecced
>    on purpose; see the note at the end of §5.
>
> **Cost, measured rather than estimated:** with the spec bundle handed to each worker instead of fetched,
> billed volume fell **6.9×**. One soul's card costs ~51K tokens; each appearance of a soul in a scene
> costs ~107K. Against the 2M crew budget that is roughly **15 soul-appearances** for the whole slice,
> before revisions — which is what makes the revision cap load-bearing rather than tidy.
> Where this file and the note above differ, the note governs.
...
- **Live mode** — **player-BYOK**; the pipeline **re-runs between runs (boot / run-end), never during a
  run**, and the deepest-bond "wow" beat is one of its outputs (H10, refined 2026-07-19). Requires an
  explicit human-approval gate before any content is written [`recon.md` §4 Pattern D].
...
[`lens-3` Agent 1; `GATE-2-review.md` B3; `coverage-map.md` §2 H1 ○○○].
...
formal I/O precedent [`lens-3` §9 flag 1; `coverage-map.md` §3.4].
...
**Modes:** canned primarily; **live** (player-BYOK) for the deepest-bond wow beat, generated **between runs**
(never during a run; requires the human gate before write) [`recon.md` §4; `gdd-structure-model.md` Build §5;
`lens-3` Agent 2].
...
- **C1 knowledge-travels** — knowledge carries across scenes/years with a soft in-world reminder
  [`GATE-2-review.md` C1];
- **voice register** — lines match the flat-register contract [`voice-style-guide`]. **Three line items,
  never collapsed:** the shared world dialect; this soul's specific signature and no other's; and
  **warmth invariance** — where a card sets a register *spread*, the spread governs tempo and uptake
  only, so a line at the flat end that reads brusque, clipped, dismissive or transactional is a flag
  even though it satisfies "flat and short" [proof run 2026-07-25];
- **delta ceiling** — two facts per scene, one WORLD and one PERSONAL. Hard: the Architect may not raise
  it to fit its own content, and a request to exceed it surfaces to the human gate [proof run 2026-07-25];
- **slot typing** — every item declares `slot_type`; narration inside a `dialogue` slot is a flag, a seed
  that is an unmentioned act belongs in `action`, and an `action` with no unambiguous actor is a flag
  [proof run 2026-07-25].

**A property of this agent, learned by running it: it is criteria-bound, not judgment-bound.** It finds
what it is given a check for and nothing else — in the proof run it caught every defect it had a check
for, including two of its own earlier clearances once new criteria were supplied, and missed the one
defect no check covered. Two consequences. Any condensing of its inputs must be a **complete distillation
of the checks, not a truncation** — a dropped check is a class of defect that becomes invisible. And it
carries an **`uncovered`** flag type, for naming a defect it can see but has no check for.
...
```json
{
  "new_lines": [ { "content_id": "string", "speaker_id": "string", "slot_type": "dialogue | action | object", "tone": "string", "text": "string", "scene_id": "string" } ],
  "active_canon": {
  ...
  "invariant_set": ["superposition", "role_boundary", "essence_vs_role", "informational_feedback", "knowledge_travels", "voice_register", "delta_ceiling", "slot_typing"]
}
```
**Output (JSON sketch):**
```json
...
current tag→asset library and produces a compliant **Unreal GameplayTag** per required audio trigger (a
hierarchical, extensible tag mapped to a **Wwise event** in the library), checking each proposed tag against
the library for collisions and orphan/missing mappings. It generates **no audio content** and assigns **no** audio style or emotion
...
[`coverage-map.md` §2 H15]); its I/O is the cleanest in the crew (string generation against a fixed pattern +
...
**KB grounding.** The USP spec — **GameplayTags + a tag→asset library + direct Wwise-event targeting** (H15,
refined 2026-07-19; superseding the old string-pattern + mirrored-tree) — is Build §8–9 [`gdd-structure-model.md` Build §8–9]. The role is
...
```json
{
  ...
  "required_interactions": ["string — from the locked four-family verb grammar (Collect/Make/Show-Ask/Use); sub-verbs slot in from the game-34 verb-table"],
  "existing_tag_library": "path — current GameplayTag → Wwise-event library (DataAsset/DataTable)"
}
```
**Output (JSON sketch):**
```json
{
  "new_tags": [
    {
      "entity_id": "string",
      "gameplay_tag": "string — department-agnostic GameplayTag, e.g. NPC.Chef.ShowAsk.React (no Audio. prefix; text/art key off the same tag)",
      "wwise_event": "string — the mapped Wwise event",
      "collision_flag": "bool — true if the tag already exists in the library",
      "orphan_flag": "bool — true if the tag has no Wwise-event mapping"
    }
  ],
  "library_delta": "path — updated tag→asset library with new entries appended",
  "violations": ["string — any proposed tag that fails the GameplayTag naming convention"]
}
```
...
**Dependency flag.** The `required_interactions` field inherits from the four-family verb grammar
(Collect / Make / Show-Ask / Use), but the **sub-verb list within each family is not yet decided** (parked
in Build §4). The Audio-Tag Agent can only emit compliant tags once the sub-verb list is specced — a
named Phase-3 dependency on the pnc-grammar / verb-table work [`gdd-structure-model.md` Build §4;
`coverage-map.md` §2 H5 ●●○; `lens-3` §9 flag 3].
...
run without an LLM at play time**, so remembering incarnations, tracking **bond level**, and gating the
...
- **Project Manager (task-board agent).** *Feature:* decomposes `gdd/` into a task board, tracks
...
- **Audio Implementer (sound-hookup agent).** *Feature:* consumes the Audio-Tag manifest, wires each tag to
  ...
  *implements* against it and inventories the assets. *Capability:* medium — engine integration
  (Wwise / Unreal) is partly deterministic wiring, so this is likely a **mechanics + agent hybrid**
...
| 4 | **Audio-Tag Agent** | Audio-tag contract — **GameplayTags + tag→asset library** (H15 USP) | Schema | canned | Soft — library delta reviewed, auto-commits on no-objection |
...
\*Agent 2 serves live mode (player-BYOK) for the deepest-bond wow beat, generated between runs; that call requires the
...
## 10. How this maps to the current GDD

This artifact supplies the dev-crew pipeline roster now living in [`../../gdd/11-ai-agents-and-pipeline.md`](../../gdd/11-ai-agents-and-pipeline.md) (H11): each agent above becomes one roster row — name · role · input schema · output schema · when-called · human-gate · realistic-capability check. In-game runtime agents (H10, a separate architecture) are scoped out here (§8). Any remaining gaps are tracked in `PAUSED.md`'s open-items list rather than a numbered GDD ledger.
...
not schemas"* [`coverage-map.md` §3.4]. The confidence here comes from **three-lens convergence**, not KB depth
...
- **Token budget / call frequency (H13) — not derivable here.** Untouched by the KB [`coverage-map.md` §2 H13
...
- **The Audio-Tag Agent's contract format has no KB precedent** (H15 ○○○) and is blocked on the Phase-3
  verb-grammar sub-verb list [`coverage-map.md` §2 H15; `gdd-structure-model.md` Build §4]. The slot is justified;
...
- **Whether the orthogonal-trait pipeline yields perceptually distinct NPCs is unvalidated in a sketch.**
  ...
  `modular-characters-system-driven`].
- **No Unreal-specific integration.** The call-down/injection patterns are engine-agnostic; Wwise +
