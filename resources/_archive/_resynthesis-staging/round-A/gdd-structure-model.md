...
Both docs inherit the 12-section spine from `resources/GDD-template-draft.md`, kept intact
...
| 8 | Audio-First Pipeline (USP) | one line in **P2/P5** | §8 (event/tag naming contract, string-pattern altitude) |
| 9 | Project Conventions | — (never Pitch) | §9 (mirrored-tree rule as concrete strings + CI note) |
...
|---|---|---|---|---|---|
| **P1** | Elevator Pitch | T1 | 1 sentence | Player verb + emotional payoff in one line — *"a soul searching for your partner across reincarnations in a Ghibli-warm world; you read lives, deduce who they've become, and the game remembers every life you've lived"* [`concept-dig-notes` Session 3] | World backstory, tech stack, genre tags, system names (ICM, two-mode architecture) |
...
| **§4** | Core Loop & Mechanic | T2 | Each loop tier its own subsection; **receiver-determined outcomes** rule (the target holds the response logic — the action verb encodes *what was done*, never *what happened*) [`concept-dig-notes` Session 8]; park undecided sub-verbs | H4 (families); locked: four families, life-verbs, festival run-end |
...
| **§7** | Art & Audio Direction | T1 → T2 | Tone words + refs (T1); dominant-object-class + color grammar (Frieren desaturation / Ghibli palette discipline, no reproduced imagery) + **going-big trigger model** (payoff swell / sprinkled wonder; large or intimate) + sonic identity (partner leitmotif emerging from festival mix) [`GATE-2-review.md` D1; `concept-dig-notes` Session 7; `_index.md` §3.11] | **H17**; open: 2D vs 3D (Roc, P3) |
| **§8** | Audio-First Pipeline (USP) | **T3** | **The differentiator.** Event/tag naming as a **string pattern**, not prose (`<Entity>_<AnimVerb>_<State>`); auto-link rule as a directory rule; audio-as-object spec (sounds are pack-free collectibles, show-able, gift-able, spell-component candidates); deliberate-recording constraint [`concept-dig-notes` Session 8; `lens-2` §8] | **H15** (audio-tag contract — ○○○, Phase-3) |
| **§9** | Project Conventions | **T3** | The path *is* the metadata — mirrored trees `Game/Animation/<Entity>/<AnimVerb>/` ↔ `Game/Audio/<Entity>/<AnimVerb>/` as concrete strings + a CI-check note; state the dependency arrow to §8 [`lens-2` §9] | Enables §8 (convention, not a hole) |
| **§10** | Platform, Engine & Scope | T2 (engine) → T3 (slice math) | Engine = Unreal, one line (MetaSounds justifies the audio pipeline); **slice contract** (5 locked items); **content-budget formula** as a named open question (`candidates × years × slots × interactions + echoes + descriptor-lines`); Van Buren guardrail referenced not repeated [`concept-dig-notes` Session 9; `lens-2` §10] | **H12, H14, H18**; locked: Unreal, slice contract; open: slice math |
| **§11** | Milestones | T1 | Dates + deliverable, one line each; **Phase-3-blocking sub-row** per milestone (what must be spec'd before it closes) [`lens-2` §11] | 7/21 draft · 8/25 capstone |
...
- **Wave B — decided shape, needs authoring:** §8 (finalize the tag/auto-link contract
  in place — highest-value, do not shortchange), §9 (mirrored-tree rule — write
...
- §8 and §9 are coupled — §9's mirrored-tree convention is the mechanism §8's auto-link
  contract depends on.
...
- **§5 (agentic AI / dev-crew, H11) is genuinely under-sourced** — the KB supplies mindset
  and role-seeds, no I/O-contract schemas [`_index.md` §3.4]. This model tells you *where*
...
- **§8 (USP / audio-first, H15) is the highest-value, thinnest-in-KB Build section** (○○○)
  [`_index.md` §2 H15]. The template descriptions gave shape; the actual string-level
