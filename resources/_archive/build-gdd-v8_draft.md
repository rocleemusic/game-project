...
*Roles are chosen once, at save-slot creation — the player's alongside that life's present souls, dealt from what's left after the player's pick — and locked for the life. For the slice, the player's choice at creation is Mage or Blacksmith only; the full pool (#7) is the full-game set. The only way to get a different player role is a new save slot, or deleting/clearing an existing one.*
...
**Mage is not player-exclusive.** Whoever holds it is "the newcomer" — a personal goal (collect magic from around the world), not a civic one like every other role's. Cycle 1 of every life, the player is always dealt mage — guaranteed, no choice. At the turn from cycle 1 into cycle 2, the player picks their role once — first, from the full pool including mage — before the engine deals that life's present souls their roles from what's left. That pick locks for the rest of the life; it is not re-picked at later turns of the year. If the player keeps mage, they stay the newcomer; if they pick something else, mage re-enters the pool for a soul to be dealt, and that soul becomes the life's newcomer instead.
