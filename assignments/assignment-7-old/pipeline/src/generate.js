...
export async function generate(slot, card, { revision = 0, flag = null, model } = {}) {
  ...
  const user = [
    ...
    '',
    // DEMO ONLY, and declared rather than hidden. A provocation slot carries a
    // style_pressure that pushes the generator toward a specific wrong shape, so the
    // evaluator has something real to catch. This is the assignment's own method:
    // "Feed the Generator three separate prompts designed to produce wrong content."
    // Production slots carry no such field, and the refine pass never sees it — a
    // revision that kept re-reading the pressure could never converge.
    slot.style_pressure && revision === 0
      ? ['## Additional direction for this slot', '', slot.style_pressure, ''].join('\n')
      : null,
