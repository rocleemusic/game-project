...
// look around (flavor, once) or open the calendar. The calendar NEVER
...
// home_hub_final below, and only the explicit "Open the calendar" pick
// moves on. A bare day-loop walk spends one extra step here per evening,
// plus one more the single time the once-only look-around is available
// (walk.test.ts).
=== home_hub ===
...
=== final_screen ===
Placeholder: festival summary and results — festival tier, bonds, threads (wired once role_goals_advanced compiles). #screen:FS #id:SYS-FINAL-SUMMARY
Start a new game to live it again. #id:SYS-FINAL-RESTART
-> END

// Advances the block, NOT called per move (emitMoveTo only calls this
// once a block's move budget is exhausted) — and only ever from morning
// or afternoon; an exhausted evening diverts straight to day_end instead
// (see emitMoveTo), so this never needs to reach `night`. Night is
// festival night, the final sequence (see home_hub_final above), not a
// normal block. A temp holds the block being left, so a single call
// cannot cascade two steps.
