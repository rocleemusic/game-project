...
export function searchReachable(inputs: WalkInputs, options: SearchOptions = {}): SearchResult {
  ...
  for (const bias of ["max", "min"] as BondBias[]) {
    ...
    while (weeksThisPass < opts.maxWeeks) {
      if (Date.now() > deadline) {
        ...
        reasons.push(
          `TIME BACKSTOP: wall clock exceeded ${opts.maxMillis}ms during the "${bias}" pass after ` +
      ...
      const run = exploreWeek(inputs, plan, seenChoices, seenOptions, opts, bias);
      weeks += 1;
      weeksThisPass += 1;
      paths += run.paths;
  ...
  const runPlan = (built: WeekPlan & { capped: boolean }, label: string): string => {
    ...
    if (!walkedPlans.has(key)) {
      const run = exploreWeek(inputs, built, seenChoices, seenOptions, opts, "max");
      paths += run.paths;
